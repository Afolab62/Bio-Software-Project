...
class DataStore {
  ...
  // Experiment methods
  createExperiment(data: Omit<Experiment, 'id' | 'createdAt' | 'updatedAt'>): Experiment {
  ...
  getExperimentsByUser(userId: string): Experiment[] {
    ...
    return userExperiments.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
  }
  ...
  getAllData() {
    return {
      users: Array.from(this.users.values()).map(u => ({ ...u, passwordHash: '[REDACTED]' })),
